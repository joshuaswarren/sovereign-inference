...
## 2026-06-30 — Desktop app: first-run onboarding + unified app server

**Done** (branch `feat/desktop-onboarding`, PR pending)
- Made the desktop app **fully usable**, not just a packaged shell. A new unified
  **app server** sidecar (`sip-app-server`) serves the OpenAI endpoint, node status,
  the dashboard, and an **onboarding/admin API** on one loopback origin, with a
  routing backend **rebuilt live** on config change and config **persisted** to the
  OS app-data dir (restored on boot).
- **First-run wizard** (`apps/dashboard/src/onboarding`): use a local model
  (Ollama / llama.cpp, fronted in-process via the real gateway → verified receipts)
  and/or connect to network providers/directories, with a built-in test chat. The
  dashboard is gated behind onboarding so its panels never load unconfigured.
- **Security**: a signed-`manifest_uri` trust boundary (`sources.py`), SSRF guard,
  trust-on-first-use endpoint pinning, and a per-install admin token on mutating
  `/api` routes (generated by the shell, injected into the webview).
- **Lifecycle**: window gated on a health check; sidecar killed on exit **and** a
  parent-death watchdog so it never orphans (verified under SIGKILL).
- **Packaging**: a PyInstaller spec bundles catalog/schemas/dashboard/adapter
  metadata with a `_MEIPASS`-aware resolver; verified by a frozen smoke test and a
  live run doing real local inference with a verified receipt.
- Shared `sip_router.in_process_client` (`ThreadedASGITransport`); refactored the
  privacy demo onto it. 520 Python tests + 6 vitest tests; ruff + mypy --strict clean.
